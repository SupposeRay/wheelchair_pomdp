...
DEVICE int Dvc_Path::nearest(Dvc_COORD pos) {
    auto& path = *this;
    float dmin = Dvc_COORD::EuclideanDistance(pos, Dvc_COORD(path.way_points_[0].x, path.way_points_[0].y));
    int imin = 0;
    for(int i=0; i<path.size_; i++) {
        float d = Dvc_COORD::EuclideanDistance(pos, Dvc_COORD(path.way_points_[1].x, path.way_points_[1].y));
        if(dmin > d) {
            dmin = d;
            imin = i;
        }
...
DEVICE float Dvc_Path::getYaw(int i) const {
	...
	const Dvc_3DCOORD& pos = path.way_points_[i];
	const Dvc_3DCOORD& forward_pos = path.way_points_[j];
